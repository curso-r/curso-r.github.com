...
```{r}
siglas <- read.xlsx('~/Downloads/atlas2013_dadosbrutos_pt.xlsx', sheet=1)
mun <- read.xlsx('~/Downloads/atlas2013_dadosbrutos_pt.xlsx', sheet=2)
est <- read.xlsx('~/Downloads/atlas2013_dadosbrutos_pt.xlsx', sheet=3) %>%
  tbl_df %>%
  filter(ANO==2010) %>%
  select(uf=UF, ufn=UFN)

mun <- mun %>% inner_join(est)
...
names(mun) <- rm_accent(tolower(names(mun)))
siglas$SIGLA <- rm_accent(tolower(siglas$SIGLA))
names(siglas) <- rm_accent(tolower(gsub('\\.', '_', names(siglas))))
