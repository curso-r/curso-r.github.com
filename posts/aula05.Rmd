...
Observe que o primeiro argumento da função `ggplot` é um data frame. A função `aes()` descreve como as variáveis são mapeadas em aspectos visuais das formas geométricas definidas pelos *geoms*. A combinação dessas duas camadas define o tipo de gráfico que você deseja construir.
...
No exemplo acima, atribuímos aspectos de posição: ao eixo y mapeamos a variável `mpg` (milhas por galão) e ao eixo x a variável `disp` (cilindradas). Como cada par será representado por um ponto, devido a utilização da função  `geom_point`, nós construímos um gráfico de dispersão. Se nesse gráfico quisermos visualisar também como essa relação se comporta em cada tipo de transmissão (variável `am`), podemos acrescentar outro aspecto visual: a cor.
